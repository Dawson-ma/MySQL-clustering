...
class DBSCAN_clustering final {
    ...
    using TVector = std::vector<std::vector<double>>;
    using DistanceFunc = std::function<double(const std::vector<double>&, const std::vector<double>&)>;
...
public:
    ...
    */
    int Run(TVector* V, const uint dim, const double eps, const uint min, const DistanceFunc& disfunc = [](const std::vector<double>& t1, const std::vector<double>& t2)->double { return 0; });
...
private:
    ...
    std::vector<std::vector<double>>* _data;  //Not owner, just holder, no responsible for deallocate


};

int DBSCAN_clustering::Run(TVector* V, const uint dim, const double eps, const uint min, const DistanceFunc& disfunc) {
    // Validate
...
void DBSCAN_clustering::destroyKdtree() {
    kd_free(this->_kdtree);
}

void DBSCAN_clustering::buildKdtree(const TVector* V)
{
...
std::vector<uint> DBSCAN_clustering::regionQuery(const uint pid) const {
    ...
    while (!kd_res_end(presults)) {
        ...
        std::vector<double>* pch = (std::vector<double>*)kd_res_item(presults, v.get());
...
void DBSCAN_clustering::expandCluster(const uint cid, const std::vector<uint>& neighbors) {
...
void DBSCAN_clustering::addToCluster(const uint pid, const uint cid) {
